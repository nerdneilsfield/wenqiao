...
顺带说一句，这个 benchmark 本身的“量级”也值得记住：原始序列一共有 39 段，Kinect 的 RGB-D 数据是 640$\times$480、30 Hz，动捕系统用 8 台高速相机给 100 Hz 的真值轨迹；除了手持序列，也有 Pioneer 3 机器人挂载 Kinect 的轨迹 [Sturm et al.](cite:sturmBenchmarkEvaluationRGBD2012)。因此 AA-ICP 在这个数据上加速的，主要是“室内、连续帧、初值不算太离谱”的那一类匹配。
...
<!-- ai-prompt: |
  ...
  Step 3 "SVD 分解":
  - Show "H = U Σ V^T" as three blocks; Σ is diagonal with symbolic σ1$\geq$σ2$\geq$σ3 (no numeric values).
...
**与 SVD 等价性的深层含义**：两种方法的“分解部分”确实是固定大小矩阵（3$\times$3 SVD 或 4$\times$4 特征分解），代价基本可视为常数；但不要把它误读成“与点数无关”。构造质心与交叉协方差 $H$ 仍是 $O(n)$ 的线性遍历，只是这一步的常数非常小，通常远小于一次 KD-tree 最近邻查询的开销。在多数工程实现里，变换估计步很少是瓶颈，瓶颈更常出现在对应搜索、法向/协方差估计或鲁棒核的权重更新上。
...
| C²IM-NN [Shin et al.](cite:shin2IMNN2025) | 28 nm CAM-PIM | 3D 点云匹配 | 能效、存储占用 | 23.08$\times$ 能效提升，48.4\% 存储占用下降 |
...
C²IM-NN 利用模拟 CAM 实现近似相似度搜索：让每个存储单元直接参与"查询点与存储点有多接近"的比较，由片内电路输出候选结果，从而持续减少数字域中的显式数据搬运和逐点比较。在此基础上，C²IM-NN 还引入轻量级 1D-CNN 在搜索前预测查询更可能落在哪个区域，再让 CAM 只在该区域内执行相似匹配——这一剪枝的前提是区域预测本身足够轻量，且误判率不能把后续搜索精度拖垮。在 28 nm CMOS 验证下，C²IM-NN 相对之前的 ASIC 加速器达到 23.08 倍能效提升，并减少 48.4\% 存储占用 [Shin et al.](cite:shin2IMNN2025)。这一结果说明，若允许引入模拟匹配和预测剪枝，PIM 的收益可以继续扩大；但代价是设计会明显更依赖工艺、校准和可靠性控制。



![C²IM-NN CAM 模拟搜索与 1D-CNN 预测剪枝流程](images/c2im-cnn-pruning.png)
...
<!-- ai-prompt: |
  Global style constraint: academic publication figure, white background, flat vector graphics, clear hierarchy, Chinese-first labels, consistent semantic colors (#2B4C7E compute/process, #3C7A89 memory/storage, #D17A22 data flow/intermediate state, #5B8C5A valid/retained region, #A23B2A conflict/failure), balanced whitespace, and multi-panel layout when useful; if no verified numeric data are available, render as a conceptual mechanism diagram rather than a quantitative chart.
  Three-panel C²IM-NN concept figure.
...
![PIM 架构原理与 PICK/C²IM 对比](images/pim-architecture.png)
...
<!-- ai-prompt: |
  ...
  Compare a traditional processor-plus-DRAM pipeline, PICK-style SRAM-PIM, and C²IM-NN-style CAM-PIM.
...
| C²IM-NN [Shin et al.](cite:shin2IMNN2025) | CAM-based PIM | 模拟相似匹配 + 1D-CNN 预测剪枝 | 23.08$\times$ 能效提升，48.4\% 存储占用下降 | 更依赖模拟电路设计、工艺校准和系统可靠性控制 |
...
当数据搬运功耗在系统总功耗中的占比持续上升时，将距离计算与 Top-$k$ 维护下推到存储阵列内部是消除带宽墙的根本路径。PICK 利用 SRAM 内部并行直接完成 kNN 搜索，在 KITTI、S3DIS、DALES 等数据集上相对既有设计实现 4.17 倍加速和 4.42 倍节能[Nie et al.](cite:niePICKSRAMbasedProcessinginmemory2025)。C²IM-NN 基于 9T1C SRAM 单元设计行方向内容寻址存储与列方向存内计算，并以 1D-CNN 预测体素点数将外部内存访问复杂度从 $O(N^2)$ 降至 $O(N)$；在 28 nm CMOS 工艺下功耗仅 137.41 mW，相比 FPGA 基线方案功耗降低 99.51%，能效提升 23.08 倍[Shin et al.](cite:shin2IMNN2025)。
